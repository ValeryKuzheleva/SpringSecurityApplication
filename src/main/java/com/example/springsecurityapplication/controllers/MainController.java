...
//        System.out.println(personDetails);
    ...
    public String registration(@ModelAttribute("person") Person person) { //1 вариант
        return "registration";
    }
//    @GetMapping("/registration") //2 вариант
...
//    }
        ...
        if (!ot.isEmpty() & !Do.isEmpty()) {
            if (!price.isEmpty()) {
                if (price.equals("sorted_by_ascending_price")) {
                    ...
                    model.addAttribute("search_product", productRepository.findByOtAndDo(Float.parseFloat(ot), Float.parseFloat(Do)));
                }
            } else {
    ...
    public String addProductInCart(@PathVariable("id") int id, Model model){
        ...
        //Извлекаем обьект аутентификации пользователя
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        //Взяли из аутентификации модель обьекта person
        PersonDetails personDetails = (PersonDetails) authentication.getPrincipal();
        //Извлекаем id пользователя из обьекта
